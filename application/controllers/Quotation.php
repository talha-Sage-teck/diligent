...
class Quotation extends Front_Controller {
	...
	public function quotation_list() {
		...
		// Assuming the current client's ID is stored in the session or accessible via $this->client_id
		$current_client_id = $this->session->userdata('client_id'); // Adjust this based on your session setup
		
		// Add a condition to filter quotations by the current client
		if ($current_client_id) {
			$condition['client_id'] = $current_client_id; // Assuming 'client_id' is the relevant column
		}
	
		// If the user is an agent, further filter by the agent who prepared the quotation
		if ($this->ion_auth->in_group('agent')) {
			$condition['prepared_by'] = $this->user_id; // Filter by the current agent's user ID
		}
	
		// Add status filter if one is provided
		if ($filter != "") {
			$condition["status"] = $filter;
		}
	
		// Fetch the list of quotations based on the condition
