...
class Users extends Admin_Controller
{
    function __construct()
    {
		...
		$this->load->model('users_additional_model');
		$this->load->model('notification_model');
		$this->load->model('users_packages_model');
		$this->load->model('earnings_log_model');
		$this->load->model('wallet_log_model');
		$this->load->model('inbox_model');
		$this->load->model('deposit_log_model');
    ...
    public function read($id) 
    {
        ...
        if ($row) {			 
			$data['user_info'] = $row;
			$data['user_additional'] = $this->users_additional_model->get_all(array('user_id' => $row->id), FALSE);
			$data['referred_users'] = $this->users_model->get_all(array('referrer_id' => $row->id));
			$data['cur_packages'] = $this->users_packages_model->get_current_plan($row->id);
			$data['cur_earnings'] = $this->earnings_log_model->earningsByUser($row->id);
			$data['cur_bonus'] = $this->wallet_log_model->userTotalBonus($row->id);
			$data['deposit_history'] = $this->deposit_log_model->get_all(array('user_id' => $row->id));
			$data['withdraw_history'] = $this->withdraw_log_model->get_all(array('user_id' => $row->id, 'request_status' => 2));
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
			'action' => 'Add',
            'frm_action' => admin_url('users/create_action'),
		    'id' => set_value('id'),
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
				'username' => $this->input->post('email',TRUE),  // put email as username
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',				
				'action' => 'update',
				'frm_action' => admin_url('users/update_action')
	    );
